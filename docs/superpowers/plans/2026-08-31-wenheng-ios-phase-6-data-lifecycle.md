...
- [ ] **Step 3: 实现只读 `information_schema` 审计服务**
...
- [ ] **Step 4: 实现确定性扫描父记录并复用 Worker 步骤**
...
- [ ] **Step 1: 写失败的 Web 会话与原生 Keychain 凭证测试**
...
- [ ] **Step 2: 扩展现有原生安全插件的独立 Keychain 项**
...
E2E 用 `page.route('**/api/**', handler)` 返回完整 API 信封；登录、菜单、预览、申请、状态和取消均使用固定夹具。测试在页面加载前通过 `page.exposeFunction` 提供内存 Keychain 状态，再把 `window.Capacitor.Plugins.WenhengSecureSession` 的六个方法桥接到该状态，使刷新/新文档仍能恢复凭证；这只是浏览器中的最小原生桥接桩。测试标题和验证记录必须同时说明“本地模拟 API + 模拟 Keychain 桥接”，不能写成真机验证。

- [ ] **Step 4: 运行 Web 完整验证矩阵**
...
- [ ] **Step 5: 在两个手机视口执行注销闭环验收**

Run: `pnpm -C apps/web exec playwright test e2e/mobile-account-deletion.phase6.spec.ts --project=chromium`

验收固定覆盖 375×667 与 393×852：立即删除、30 天删除、取消、状态令牌恢复、网络中断、杀页面后恢复、受限保留文案、无横向溢出和控制台错误为 0。API 使用与生产相同的响应信封和状态枚举，不把前端 mock 成“已删除真实数据”。

- [ ] **Step 6: 同步 Capacitor 并构建无签名通用模拟器**
...
```bash
git add apps/backend/src/modules/privacy-lifecycle/services/lifecycle-e2e.service.test.ts apps/web/e2e/mobile-account-deletion.phase6.spec.ts apps/web/playwright.config.ts apps/web/package.json pnpm-lock.yaml docs/verification/2026-08-31-wenheng-ios-phase-6.md docs/superpowers/specs/2026-08-31-wenheng-data-lifecycle-design.md docs/superpowers/plans/2026-08-31-wenheng-ios-phase-6-data-lifecycle.md
