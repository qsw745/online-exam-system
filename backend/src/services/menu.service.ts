...
export class MenuService {
  ...
  static async createMenu(menuData: CreateMenuRequest): Promise<number> {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      ...
      [
        name, title, path, component, icon, parent_id, sort_order, level,
        is_hidden, is_disabled, false, menu_type, permission_code,
        redirect, meta ? JSON.stringify(meta) : null, description
      ]
    );
  ...
  static async createRole(roleData: CreateRoleRequest): Promise<number> {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      'INSERT INTO roles (name, code, description, is_system, is_disabled) VALUES (?, ?, ?, ?, ?)',
      [name, code, description, false, false]
    );
