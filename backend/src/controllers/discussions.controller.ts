...
export class DiscussionsController {
  // 获取讨论列表
  static async getDiscussions(req: AuthRequest, res: Response<ApiResponse<{ discussions: IDiscussion[]; total: number; categories: IDiscussionCategory[] }>>) {
    try {
      ...
      switch (sort) {
        case 'hot':
          orderClause = 'ORDER BY d.is_pinned DESC, (d.like_count + d.reply_count * 2 + d.view_count * 0.1) DESC, d.created_at DESC';
          break;
        case 'replies':
      ...
      const queryParams = userId ? [userId, userId, userId, ...params, parseInt(limit as string), offset] : [...params, parseInt(limit as string), offset];
  ...
  // 获取讨论详情
  static async getDiscussionDetail(req: AuthRequest, res: Response<ApiResponse<{ discussion: IDiscussion; replies: IDiscussionReply[] }>>) {
    try {
      const userId = req.user?.id;
      ...
      const repliesQuery = `
        SELECT dr.*, u.username, u.avatar_url as avatar,
               ${userId ? `EXISTS(SELECT 1 FROM discussion_likes dl WHERE dl.user_id = ? AND dl.target_type = 'reply' AND dl.target_id = dr.id) as is_liked` : 'FALSE as is_liked'}
      ...
      const repliesParams = userId ? [userId, discussionId, parseInt(limit as string), offset] : [discussionId, parseInt(limit as string), offset];
    ...
    try {
      ...
      const [result] = await pool.query<ResultSetHeader>(
        `INSERT INTO discussions (user_id, category_id, title, content, tags, related_type, related_id, last_reply_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
        [userId, category_id || null, title, content, JSON.stringify(tags || []), related_type || 'general', related_id || null]
      );
  ...
  static async updateDiscussion(req: AuthRequest, res: Response<ApiResponse<null>>) {
    try {
      ...
      // 检查权限
      const [discussions] = await pool.query<IDiscussion[]>(
      ...
      await pool.query(
        'UPDATE discussions SET title = ?, content = ?, category_id = ?, tags = ? WHERE id = ?',
        [title, content, category_id || null, JSON.stringify(tags || []), discussionId]
      );
  ...
  static async createReply(req: AuthRequest, res: Response<ApiResponse<{ reply_id: number }>>) {
    try {
      ...
      if (parent_id) {
        await pool.query(
          'UPDATE discussion_replies SET reply_count = reply_count + 1 WHERE id = ?',
          [parent_id]
        );
      }
  ...
  static async toggleLike(req: AuthRequest, res: Response<ApiResponse<{ is_liked: boolean; like_count: number }>>) {
    try {
      ...
      if (existing.length > 0) {
        // 取消点赞
        await pool.query(
          'DELETE FROM discussion_likes WHERE user_id = ? AND target_type = ? AND target_id = ?',
          [userId, target_type, target_id]
        );
        isLiked = false;
      } else {
        // 添加点赞
        await pool.query(
          'INSERT INTO discussion_likes (user_id, target_type, target_id) VALUES (?, ?, ?)',
          [userId, target_type, target_id]
        );
        isLiked = true;
      }
  ...
  static async toggleBookmark(req: AuthRequest, res: Response<ApiResponse<{ is_bookmarked: boolean }>>) {
    try {
      ...
      if (existing.length > 0) {
        ...
        // 添加收藏
        await pool.query(
          'INSERT INTO discussion_bookmarks (user_id, discussion_id) VALUES (?, ?)',
          [userId, discussionId]
        );
        isBookmarked = true;
      }
  ...
  static async getUserStats(req: AuthRequest, res: Response<ApiResponse<any>>) {
    try {
      ...
      const [stats] = await pool.query<RowDataPacket[]>(
        'SELECT * FROM user_discussion_stats WHERE user_id = ?',
        [userId]
      );
