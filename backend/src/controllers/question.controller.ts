...
export class QuestionController {
  static async list(req: AuthRequest, res: Response<ApiResponse<QuestionListData>>) {
    try {
      const question_type = req.query.type as 'single_choice' | 'multiple_choice' | 'true_false' | 'short_answer' | undefined;
  ...
  static async create(req: AuthRequest, res: Response<ApiResponse<QuestionData>>) {
    try {
      const { content, question_type, options, correct_answer, exam_id, score = 10 } = req.body;
      
      if (!content || !question_type || !options || !correct_answer || !exam_id) {
        const response: ApiResponse<QuestionData> = {
          success: false,
          error: '缺少必填字段'
        };
      ...
      const [result] = await pool.query<ResultSetHeader>(
        'INSERT INTO questions (content, question_type, options, correct_answer, exam_id, score) VALUES (?, ?, ?, ?, ?, ?)',
        [content, question_type, options, correct_answer, exam_id, score]
      );
      
      const [question] = await pool.query<IQuestion[]>(
        'SELECT * FROM questions WHERE id = ?',
        [result.insertId]
      );
      ...
      await LoggerService.logUserAction({
        ...
        details: { questionType: question_type, title: title || content.substring(0, 50) },
        ipAddress: req.ip,
  ...
  static async bulkImport(req: AuthRequest, res: Response<ApiResponse<{ success_count: number; fail_count: number; errors: string[] }>>) {
    try {
      ...
      for (let i = 0; i < questions.length; i++) {
        try {
          ...
          const { content, question_type, options, answer, knowledge_points, explanation } = question;
          ...
          // 插入题目
          await pool.query(
            'INSERT INTO questions (title, content, question_type, options, correct_answer, knowledge_points, explanation, exam_id, score) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [title, content, question_type, optionsJson, answer, knowledgePointsJson, explanation || '', null, 1]
          );
  ...
  static async getKnowledgePoints(req: AuthRequest, res: Response) {
    try {
      ...
      const knowledgePoints = rows
        .map(row => row.knowledge_point)
        .filter(point => point && point !== 'null');
