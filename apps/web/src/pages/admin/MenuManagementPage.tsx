...
import { Card, Tree, Button, Modal, Form, Input, Select, Switch, Space, Popconfirm, Table, Tag, App, InputNumber } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined, DragOutlined, HolderOutlined, CopyOutlined, ExportOutlined, ImportOutlined } from '@ant-design/icons';
    ...
    <Card
      ...
      extra={
        <Space>
          ...
          <Button icon={<DragOutlined />} onClick={() => {
      ...
      <Tree
        ...
        onDrop={async (info) => {
          ...
          try {
            // 计算新的排序值
            let newSortOrder = 0;
            const targetMenu = menus.find(m => m.id === Number(node.key));
            const draggedMenu = menus.find(m => m.id === Number(dragNode.key));
            
            if (!targetMenu || !draggedMenu) return;
            
            if (dropToGap) {
              // 拖拽到节点之间
              if (dropPosition === -1) {
                // 拖拽到目标节点之前
                newSortOrder = Math.max(0, targetMenu.sort_order - 1);
              } else {
                // 拖拽到目标节点之后
              ...
              // 拖拽到节点内部（作为子节点）
              const childMenus = menus.filter(m => m.parent_id === targetMenu.id);
              newSortOrder = childMenus.length > 0 ? Math.max(...childMenus.map(m => m.sort_order)) + 1 : 0;
            }
            ...
            const updateData = {
              ...
              ...(dropToGap ? {} : { parent_id: targetMenu.id })
            };
            
            const response = await api.put(`/menu/menus/${draggedMenu.id}`, updateData);
    ...
    <div className="p-6">
      ...
      >
        <Form form={menuForm} layout="vertical">
          ...
          <Form.Item
            name="sort_order"
            label="排序号"
            tooltip="数值越小排序越靠前，留空将自动设置为最大排序号+1"
          >
            <Input 
              type="number" 
              placeholder="留空自动排序" 
      ...
      >
        <div className="space-y-2">
          ...
          >
            <SortableContext
              items={batchSortMenus.map(menu => menu.id)}
