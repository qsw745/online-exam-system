...
const SmartPaperCreatePage: React.FC = () => {
  ...
  const handleDifficultyDistributionChange = (
    difficulty: keyof SmartPaperConfig['difficultyDistribution'],
    value: number
  ) => {
    setConfig(prev => ({
      ...prev,
  ...
  const validateConfig = (): string | null => {
    ...
    const totalDifficultyPercent = Object.values(config.difficultyDistribution).reduce(
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          {/* antd Button 不支持 variant，改为 type="text" */}
          <Button type="text" onClick={() => navigate('/admin/papers')} className="flex items-center gap-2">
        ...
        <div className="flex justify-end space-x-4">
          <Button onClick={() => navigate('/admin/papers')}>取消</Button>
          <Button
            type="primary"
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            {/* antd Button 不支持 variant，改为 type="text" */}
            <Button type="text" onClick={() => setStep('config')} className="flex items-center gap-2">
              <ArrowLeft className="w-4 h-4" />
              返回配置
            </Button>
            <div>
          ...
          <div className="flex gap-2">
            {/* antd Button 不支持 variant="outline"，默认按钮即为描边风格 */}
            <Button
              type="default"
            ...
            >
              {generating ? <LoadingSpinner /> : <Shuffle className="w-4 h-4" />}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2 font-medium">
                {(config.totalScore / generatedQuestions.length).toFixed(1)}分/题
              </span>
            </div>
          </div>
        </Card>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="px-2 py-1 bg-gray-100 text-gray-800 text-xs rounded">{question.difficulty}</span>
                      <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">{question.score}分</span>
                    </div>
