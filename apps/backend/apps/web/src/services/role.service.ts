...
export class RoleService {
   ...
   * 分页获取角色列表（支持搜索）
   */
  static async getRolesWithPagination(page: number, pageSize: number, keyword?: string): Promise<{ roles: Role[], total: number }> {
  ...
  static async createRole(roleData: CreateRoleRequest): Promise<Role> {
    ...
    // 检查角色名称是否已存在
    const [existingRoles] = await pool.execute<RowDataPacket[]>(
