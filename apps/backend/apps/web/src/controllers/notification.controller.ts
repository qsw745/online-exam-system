...
export class NotificationController {
  ...
  static async create(req: AuthRequest, res: Response<ApiResponse<INotification>>) {
    try {
      ...
      const [notification] = await pool.query<INotification[]>(
  ...
  // 系统通知模板
  static async createSystemNotification(type: 'exam_start' | 'exam_end' | 'grade_published' | 'task_assigned', data: any) {
    try {
      const templates = {
        exam_start: {
          title: '考试开始提醒',
      ...
      const values = data.userIds.map((userId: number) => [
        userId,
        template.title,
        template.content,
        'info'
