...
export class UserController {
  ...
  static async getCurrentUser(req: AuthRequest, res: Res) {
    try {
  ...
  static async updateCurrentUser(req: AuthRequest, res: Res) {
    try {
      if (!req.user?.id) return res.unauthorized('未授权')
  ...
  static async update(req: AuthRequest, res: Res) {
    try {
      const id = Number(req.params.id)
      if (!Number.isFinite(id)) return res.badRequest('无效的用户ID')
  ...
  static async updateStatus(req: AuthRequest, res: Res) {
    try {
      const id = Number(req.params.id)
      if (!Number.isFinite(id)) return res.badRequest('无效的用户ID')
  ...
  static async resetPassword(req: AuthRequest, res: Res) {
    try {
      const id = Number(req.params.id)
      if (!Number.isFinite(id)) return res.badRequest('无效的用户ID')
  ...
  static async getSettings(req: AuthRequest, res: Res) {
    try {
      if (!req.user?.id) return res.unauthorized('未授权')
  ...
  static async saveSettings(req: AuthRequest, res: Res) {
    try {
      if (!req.user?.id) return res.unauthorized('未授权')
