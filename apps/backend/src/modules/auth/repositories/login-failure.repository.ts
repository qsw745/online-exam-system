...
export const LoginFailureRepository = {
    ...
    async upsert(email: string, ip: string, patch: Partial<Row>) {
        const conn = await pool.getConnection()
        try {
            await conn.beginTransaction()
            const [rows] = await conn.query<Row[]>(
                `SELECT id FROM ${TABLE} WHERE email=? AND ip=? LIMIT 1`,
                [email, ip]
            )
            if (rows[0]) {
                const fields: string[] = []
                const values: any[] = []
                if (typeof patch.fail_count === 'number') { fields.push('fail_count = ?'); values.push(patch.fail_count) }
                if (patch.last_failed_at !== undefined) { fields.push('last_failed_at = ?'); values.push(patch.last_failed_at) }
                if (patch.locked_until !== undefined) { fields.push('locked_until = ?'); values.push(patch.locked_until) }
    ...
    async increase(email: string, ip: string) {
        ...
        try {
            await conn.beginTransaction()
            const [rows] = await conn.query<Row[]>(
                `SELECT id,fail_count FROM ${TABLE} WHERE email=? AND ip=? LIMIT 1`,
                [email, ip]
            )
            let next = 1
            if (rows[0]) {
    ...
    async reset(email: string, ip: string) {
        await pool.query<ResultSetHeader>(
            `DELETE FROM ${TABLE} WHERE email=? AND ip=?`,
            [email, ip]
        )
    },
