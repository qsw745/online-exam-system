...
export class TaskRepository {
    ...
    async updateTask(
        ...
        if (patch.start_time !== undefined) { fields.push('start_time = ?'); values.push(this.asDateTime(patch.start_time)) }
        if (patch.end_time !== undefined) { fields.push('end_time = ?'); values.push(this.asDateTime(patch.end_time)) }
    ...
    async getAssignedUserIds(taskId: number): Promise<number[]> {
        const [rows] = await this.db.query<RowDataPacket[]>('SELECT user_id FROM task_assignments WHERE task_id = ?', [taskId])
    ...
    private async getQuestionsForGradingByPaperId(
        ...
        const sql = `
            ...
            ORDER BY pq.\`order\` ASC
        `
        const [qs] = await conn.query<RowDataPacket[]>(sql, [paperId])
        return qs.map(r => ({
            id: Number(r.q_id),
