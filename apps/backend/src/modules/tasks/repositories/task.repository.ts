...
export class TaskRepository {
    ...
    async replaceDepartmentAssignments(taskId: number, departmentIds: number[], assignedBy: number): Promise<void> {
        await this.db.query('DELETE FROM task_department_assignments WHERE task_id = ?', [taskId])
    ...
    async list(q: TaskListQuery): Promise<TaskListResult> {
        ...
        const [rows] = await this.db.query<TaskDTO[] & RowDataPacket[]>(
            `SELECT
                 t.*,
                 e.id        AS exam_id,
    ...
    /** 👉 通过 examId + userId 找该学生收到的 taskId（用于把 examId 反查为 taskId） */
    ...
    /** 👉 老师/管理员：找任意与该考试关联的 taskId（便于预览/调试） */
    async findAnyTaskIdByExam(examId: number): Promise<number | null> {
        const [rows] = await this.db.query<RowDataPacket[]>(
            `SELECT id FROM tasks WHERE exam_id = ? LIMIT 1`,
            [examId]
        )
        return rows.length ? Number(rows[0].id) : null
    }
    ...
    async getExamMetaByTask(taskId: number): Promise<{
        ...
        return {
            ...
            exam_id: r.exam_id != null ? Number(r.exam_id) : null,
            paper_id: r.paper_id != null ? Number(r.paper_id) : null,
            duration: r.duration != null ? Number(r.duration) : null,
            type: String(r.type ?? 'practice'),
        }
    }

    /** ✅ 题目视图（不返回正确答案；兼容 paper_questions 的快照字段且列可选） */
  ...
  `
        ...
        const [rows] = await executor.query<RowDataPacket[]>(sql, [paperId])
        ...
        return rows.map((r) => {
            ...
            const type = stdType((r as any).q_type ?? (r as any).pq_type ?? 'single_choice')
            const content = String((r as any).q_content ?? (r as any).pq_content ?? '')
            const rawOpts = (r as any).q_options ?? (r as any).pq_options ?? null
            const score = Number((r as any).pq_score || 0)
            const order = Number((r as any).pq_order || 0)
            ...
            return {
                ...
                type,
                content,
                // 单/多选/判断：给数组；主观题：给 null（前端渲染输入框）
                options: type === 'short_answer' ? null : (opts ?? []),
                score,
                order,
            }
        })
    ...
    async ensureExamResultStandalone(
        ...
        if (exists.length) {
            const row = exists[0]
            if (!row.start_time) {
                await this.db.query('UPDATE exam_results SET status = "in_progress", start_time = NOW() WHERE id = ?', [row.id])
...
/** 将 options 解析为 string[]；兼容：数组/严格JSON/单引号JSON/分隔符纯文本/对象数组{label,content} */
function parseOptionsLoose(input: any): string[] | null {
    ...
    try {
        ...
        if (Array.isArray(v)) {
            const arr = v.map(toLabel).map((x: string) => x.trim()).filter(Boolean)
            if (arr.length) return arr
        }
    } catch {}

    // 尝试把单引号 JSON 修正为可解析 JSON
    try {
