...
export class RoleRepository {
  ...
  static async codeExists(code: string, orgId: number | null, excludeId?: number): Promise<boolean> {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(sql, excludeId != null ? [orgId, code, excludeId] : [orgId, code])
    return rows.length > 0
  }
  ...
  static async removeRoleOrg(roleId: number, orgId: number): Promise<void> {
    await pool.query(`DELETE FROM role_orgs WHERE role_id = ? AND org_id = ?`, [roleId, orgId])
  }
