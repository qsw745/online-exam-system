...
export class MenuController {
    ...
    static async getEffectiveMenus(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!orgId) {
                return void res.status(400).json({ success: false, message: '缺少 unitId/orgId（?unitId | ?orgId | x-org-id）' })
            }
    ...
    static async getUserMenuTree(req: Request, res: Response): Promise<void> {
        try {
            ...
            const tree = orgIdHeader
                ? await MenuService.getUserMenuTreeInOrg(Number(userId), orgIdHeader)
                : await MenuService.getUserMenuTree(Number(userId))
            res.json({ success: true, data: tree })
        } catch (error) {
