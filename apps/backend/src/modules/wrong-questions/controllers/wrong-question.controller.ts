...
// —— 本地扩展 Response，避免 TS2339（与中间件 responseEnvelope 保持一致）——
type Res = Response & {
...
export class WrongQuestionController {
  // ---- books ----
  static async createBook(req: Request, res: Res) {
    try {
  ...
  static async getBooks(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
  ...
  static async updateBook(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
  ...
  static async deleteBook(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
      const id = Number(req.params.id)
  ...
  // ---- questions ----
  static async addWrongQuestion(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
  ...
  static async getWrongQuestions(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
  ...
  static async updateWrongQuestion(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
      const id = Number(req.params.id)
      ...
      const patch: any = {}
      const m = mastery_level !== undefined ? asMastery(mastery_level) : null
  ...
  static async removeWrongQuestion(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
      const id = Number(req.params.id)
  ...
  static async addPracticeRecord(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()

      let { wrong_question_id, question_id, is_correct, time_spent } = req.body
  ...
  /** 已练习题目ID列表 */
  static async getPracticedQuestions(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
  ...
  static async shareBook(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
      const id = Number(req.params.id)
  ...
  static async getSharedBook(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
  ...
  static async getStatistics(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
  ...
  static async batchAddWrongQuestions(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
      ...
      const r = await svc.batchAddWrongQuestions(
        Number(userId),
        Number(book_id),
        questions.map((q: any) => Number(q.question_id ?? q)),
  ...
  /** 批量更新掌握程度（配合路由 /questions/batch/mastery） */
  static async batchUpdateMastery(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
      const { ids, mastery_level } = req.body as { ids: Array<number | string>; mastery_level: string }

      if (!Array.isArray(ids) || ids.length === 0) return res.badRequest('ids 不能为空')
      const m = asMastery(mastery_level)
      if (!m) return res.badRequest('无效的掌握程度（not_mastered/partially_mastered/mastered）')
      ...
      for (const raw of ids) {
        ...
        if (!Number.isFinite(id)) {
          errors.push({ id: Number(raw), error: '无效ID' })
          continue
        }
        try {
          await svc.updateWrongQuestion(id, userId, { mastery_level: m })
          success++
        } catch (e: any) {
          errors.push({ id, error: e?.message || '更新失败' })
        }
      }

      return res.ok({ total: ids.length, success, failed: errors.length, errors }, '批量更新完成')
    } catch (e) {
      log.error('batchUpdateMastery error:', e)
      return res.internal('批量更新失败')
    }
  }

  static async autoCollectWrongQuestions(req: Request, res: Res) {
    try {
      const userId = (req as any).user?.id
      if (!userId) return res.unauthorized()
