// apps/backend/src/modules/exams/controllers/result.controller.ts
...
export class ResultController {
  static async list(req: AuthRequest, res: Response<ApiResponse<ResultListData>>) {
    try {
      ...
      const unauth = /未授权/.test(e?.message)
      if (unauth) return (res as any).unauthorized(e?.message || '未授权', { code: CODES.AUTH_UNAUTHORIZED })
      return (res as any).internal(e?.message || '获取考试结果列表失败', { code: CODES.INTERNAL_ERROR })
    }
  }
