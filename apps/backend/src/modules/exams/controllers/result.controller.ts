...
function buildContentDisposition(filenameUtf8: string) {
  const fallback = filenameUtf8.replace(/["\\]/g, '').replace(/[^\x20-\x7E]+/g, '_')
...
// 标准化 req.user 为 { id?: number; role?: string }
...
export class ResultController {
  static async list(req: AuthRequest, res: Response<ApiResponse<ResultListData>>) {
    try {
      const data = await svc.list(pickUser(req.user), req.query)
  ...
  static async exportCsv(req: AuthRequest, res: Response) {
    try {
      const { list } = await svc.listForExport(pickUser(req.user), req.query)
