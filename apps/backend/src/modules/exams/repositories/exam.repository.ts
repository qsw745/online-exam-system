...
export class ExamRepository {
  ...
  static async list(params: { page: number; limit: number; status?: string; search?: string }): Promise<ExamListData> {
    ...
    const [exams] = await pool.query<RowDataPacket[]>(
      `SELECT * FROM exams ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      [...vals, limit, offset]
    )
    const [tot] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) as total FROM exams ${where}`, vals)
    return {
  ...
  static async updateExam(
    ...
    try {
      ...
      const [exists] = await conn.query<RowDataPacket[]>('SELECT * FROM exams WHERE id = ? AND created_by = ?', [
        examId,
        userId,
      ])
  ...
  static async deleteExam(userId: number, examId: number): Promise<IExam> {
    ...
    try {
      await conn.beginTransaction()
      const [existed] = await conn.query<RowDataPacket[]>('SELECT * FROM exams WHERE id = ? AND created_by = ?', [
        examId,
        userId,
      ])
  ...
  static async findPublished(examId: number): Promise<IExam | null> {
    const [rows] = await pool.query<RowDataPacket[]>(
      'SELECT * FROM exams WHERE id = ? AND (status IS NULL OR status = "published")',
      [examId]
    )
    return (rows as IExam[])[0] ?? null
  }
