...
export class TaskController {
        ...
        if (task.assigned_users_info) {
          ...
          for (const userInfo of userInfos) {
            ...
            if (id && username && email) {
              assignedUsers.push({ id: parseInt(id), username, email });
            }
          }
        }
        return {
  ...
  static async get(req: AuthRequest, res: Response<TaskDetailResponse>) {
    try {
      ...
      const successResponse: TaskDetailResponse = {
        success: true,
        data: { task: processedTask }
      };
      return res.json(successResponse);
    } catch (error) {
  ...
  static async create(req: AuthRequest, res: Response<TaskDetailResponse>) {
    try {
      ...
      const userRole = req.user?.role;
      const { title, description, status, start_time, end_time, exam_id, type = 'practice', assigned_user_ids } = req.body;
      ...
      for (const userId of assignedUserIds) {
        await pool.query(
          ...
          [taskId, userId, creatorId]
        );
      }

      // 获取创建的任务信息（使用第一个分配用户的信息）
  ...
  static async update(req: AuthRequest, res: Response<TaskDetailResponse>) {
    try {
      ...
      if (userRole === 'admin' || userRole === 'teacher') {
        ...
        // 学生只能修改分配给自己的任务
        taskQuery = 'SELECT * FROM tasks WHERE id = ? AND id IN (SELECT task_id FROM task_assignments WHERE user_id = ?)';
  ...
  static async delete(req: AuthRequest, res: Response<ApiResponse<null>>) {
    try {
      ...
      if (userRole === 'admin' || userRole === 'teacher') {
        ...
        // 学生只能删除分配给自己的任务
        checkQuery = 'SELECT id FROM tasks WHERE id = ? AND id IN (SELECT task_id FROM task_assignments WHERE user_id = ?)';
    ...
    try {
      ...
      try {
        ...
        await connection.query(
          'DELETE FROM answer_records WHERE exam_result_id = ?',
          [resultId]
        );
        ...
        try {
          ...
          // 异步调用，不影响任务提交的响应
          setImmediate(() => {
            WrongQuestionController.autoCollectWrongQuestions(wrongQuestionReq as AuthRequest, {
              json: () => {},
              status: () => ({ json: () => {} })
            } as any).catch(error => {
        ...
        setImmediate(async () => {
          try {
            ...
            learningProgressController.recordProgress({
              user: req.user,
              body: {
                studyTime: studyTime,
  ...
  static async publish(req: AuthRequest, res: Response<ApiResponse<any>>) {
    try {
      ...
      if (task.status === 'published') {
        return res.status(400).json({ success: false, error: '任务已经发布' });
      }
      ...
      if (endTime <= now) {
        return res.status(400).json({ success: false, error: '结束时间不能早于当前时间' });
      }

      // 更新任务状态为已发布
      await pool.query(
        'UPDATE tasks SET status = "published", updated_at = NOW() WHERE id = ?',
        [taskId]
      );
      ...
      for (const assignment of assignedUsers) {
        await pool.query(
          ...
          [
            assignment.user_id,
            '新任务发布',
            `任务「${task.title}」已发布，请及时完成。开始时间：${startTime.toLocaleString()}，结束时间：${endTime.toLocaleString()}`
          ]
        );
      }
  ...
  static async unpublish(req: AuthRequest, res: Response<ApiResponse<any>>) {
    try {
      ...
      if (task.status === 'unpublished') {
        return res.status(400).json({ success: false, error: '任务已经下线' });
      }

      // 检查任务是否可以下线（已发布的任务才能下线）
      if (task.status !== 'published') {
      ...
      // 更新任务状态为已下线
      await pool.query(
        'UPDATE tasks SET status = "unpublished", updated_at = NOW() WHERE id = ?',
        [taskId]
      );
