...
export class PaperController {
  ...
  static async updateQuestionOrder(req: AuthRequest, res: Response<ApiResponse<null>>) {
    try {
      ...
      await Promise.all(orders.map(({ questionId, order }) =>
        pool.query(
          'UPDATE paper_questions SET `order` = ? WHERE paper_id = ? AND question_id = ?',
          [order, parseInt(id), questionId]
        )
      ));
  ...
  static async create(req: AuthRequest, res: Response<ApiResponse<PaperData>>) {
    try {
      ...
      const [paper] = await pool.query<IPaper[]>(
        'SELECT * FROM papers WHERE id = ?',
        [result.insertId]
      );
...
static async smartGenerate(req: AuthRequest, res: Response) {
  try {
    ...
    const knowledgePoints = Array.isArray(body.knowledgePoints)
      ? body.knowledgePoints.filter(Boolean)
      : [];
