  ...
  static async create(req: AuthRequest, res: Response<ApiResponse<QuestionData>>) {
    try {
      ...
      const [result] = await pool.query<ResultSetHeader>(
        ...
        [questionTitle, content, question_type, optionsJson, correct_answer, knowledgePointsJson, explanation || '', difficulty, exam_id, score]
      );
    ...
    try {
      ...
      for (let i = 0; i < questions.length; i++) {
        try {
          ...
          await pool.query(
            'INSERT INTO questions (title, content, question_type, options, correct_answer, knowledge_points, explanation, difficulty, exam_id, score) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [questionTitle, content, question_type, optionsJson, answer, knowledgePointsJson, explanation || '', difficulty, null, score]
          );
