...
router.post('/record',
  ...
  validateRequest,
  learningProgressController.recordProgress.bind(learningProgressController)
);
...
router.get('/stats',
  ...
  validateRequest,
  learningProgressController.getProgressStats.bind(learningProgressController)
);
...
router.get('/track',
  ...
  validateRequest,
  learningProgressController.getLearningTrack.bind(learningProgressController)
);

// 设置学习目标
router.post('/goals',
  [
    body('goalType').isIn(['daily_time', 'daily_questions', 'weekly_time', 'weekly_questions', 'monthly_time', 'monthly_questions', 'accuracy_rate', 'study_streak'])
      .withMessage('目标类型不正确'),
  ...
  validateRequest,
  learningProgressController.setLearningGoal.bind(learningProgressController)
);
...
router.get('/goals',
  [
    ...
    query('goalType').optional().isIn(['daily_time', 'daily_questions', 'weekly_time', 'weekly_questions', 'monthly_time', 'monthly_questions', 'accuracy_rate', 'study_streak'])
      .withMessage('目标类型不正确')
  ],
  validateRequest,
  learningProgressController.getLearningGoals.bind(learningProgressController)
);
...
router.put('/goals/:goalId',
  ...
  validateRequest,
  learningProgressController.updateGoalProgress.bind(learningProgressController)
);

// 获取学习成就
router.get('/achievements',
  learningProgressController.getLearningAchievements.bind(learningProgressController)
);
...
router.post('/achievements/unlock',
  ...
  validateRequest,
  learningProgressController.unlockAchievement.bind(learningProgressController)
);

// 获取学习报告
router.get('/report',
  [
    query('period').optional().isIn(['week', 'month', 'quarter', 'year']).withMessage('时间周期必须是week、month、quarter或year'),
    query('subjectId').optional().isInt({ min: 1 }).withMessage('科目ID必须是正整数')
  ],
  validateRequest,
  learningProgressController.getLearningReport.bind(learningProgressController)
);
...
router.get('/records',
  ...
  validateRequest,
  learningProgressController.getProgressRecords.bind(learningProgressController)
);

// 获取科目列表
router.get('/subjects',
  learningProgressController.getSubjects.bind(learningProgressController)
);
