...
export class FavoritesService {
  ...
  // 快速收藏项目到默认收藏夹
  async quickFavorite(userId: number, itemType: string, itemId: number, title?: string, description?: string): Promise<boolean> {
  ...
  // 搜索收藏项目
  async searchFavoriteItems(userId: number, keyword: string, itemType?: string, favoriteId?: number): Promise<IFavoriteItem[]> {
    try {
  ...
  // 更新收藏项目排序
  async updateItemsOrder(userId: number, favoriteId: number, itemOrders: { id: number; sort_order: number }[]): Promise<boolean> {
    const connection = await pool.getConnection();
    try {
      await connection.beginTransaction();
      ...
      for (const item of itemOrders) {
        await connection.query(
          ...
          [item.sort_order, item.id, favoriteId]
        );
      }
