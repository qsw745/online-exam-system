...
const TaskCreatePage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!title.trim()) return toast.error('请输入任务标题')
    if (!description.trim()) return toast.error('请输入任务描述')
    if (!startDate) return toast.error('请选择开始时间')
    if (!endDate) return toast.error('请选择结束时间')
    if (startDate >= endDate) return toast.error('结束时间必须晚于开始时间')
    ...
    try {
      ...
      toast.error(error.response?.data?.message || '保存任务失败')
    } finally {
      setSubmitting(false)
    }
  }
