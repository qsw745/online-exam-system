...
const DataAnalyticsPage: React.FC = () => {
  ...
  const loadOverview = async () => {
    try {
      ...
      toast.error(error.response?.data?.error || '加载概览数据失败')
      setOverview({
        totalUsers: 0,
        activeUsers: 0,
        totalSubmissions: 0,
  ...
  const loadKnowledgePoints = async () => {
    try {
      ...
      toast.error(error.response?.data?.error || '加载知识点数据失败')
      setKnowledgePoints([])
    }
  }
  ...
  const loadDifficultyData = async () => {
    try {
      ...
      toast.error(error.response?.data?.error || '加载难度数据失败')
      setDifficultyData([])
    }
  }
  ...
  const loadActivityData = async () => {
    try {
      ...
      toast.error(error.response?.data?.error || '加载活跃度数据失败')
      setActivityData([])
    }
  }
